// src/OrganizerManagementTool.jsx
import React, { useState, useEffect } from 'react';
import ExcursionDefinitionTool from './ExcursionDefinitionTool'; // Asegúrate de que la ruta sea correcta
...
const ExcursionDetailManagement = ({ excursion, onClose, onUpdate }) => {
  ...
  return (
    <div className="section" style={{ marginTop: '30px' }}>
      <h3>Gestionando: {excursion.name} (Estado: {excursion.status})</h3>
      <div style={{ marginBottom: '20px' }}>
        <button onClick={onClose} className="button secondary-button">← Volver a Mis Excursiones</button>
        <button onClick={() => console.log('Simulación: Navegar a la edición completa de la excursión')} className="button info-button" style={{ marginLeft: '10px' }}>
      ...
      <div className="tab-content">
        {activeTab === 'participants' && (
          <section className="participant-tracking">
            <h4>Listado de Participantes ({excursion.currentBookings} / {excursion.capacity})</h4>
              ...
              <table className="data-table">
                ...
                <tbody>
                  {excursion.participants.map((p, index) => (
                    <tr key={p.id} className={index % 2 === 0 ? 'table-row-even' : 'table-row-odd'}>
                      <td>{p.name}</td>
                      <td>{p.numPeople}</td>
                      <td>{p.pickupPoint}</td>
                      <td>{p.status}</td>
              ...
              {excursion.servicesBooked.length === 0 ? (
                ...
                excursion.servicesBooked.map((service, index) => (
                  <li key={index} className="list-item">
                    <span>
                      <strong>{service.name}</strong> (Costo: ${service.cost}) - Estado: {service.status}
                    </span>
                    <div>
...
const OrganizerExcursionManagementTool = () => {
    ...
    const dummyExcursions = [
      {
        ...
        galleryImages: ['https://via.placeholder.com/150?text=Rafting1'],
      ...
      },
      {
        ...
        galleryImages: ['https://via.placeholder.com/150?text=Playa1'],
        participantRecommendations: {
      ...
      },
      {
        ...
        galleryImages: ['https://via.placeholder.com/150?text=Arenal1'],
        participantRecommendations: {
  ...
  const handleBackFromDefinition = () => {
    ...
    setSelectedExcursion(null);
    console.log('Simulación: Volviendo al panel principal.');
  };
          ...
          <table className="data-table">
            ...
            <tbody>
              {myExcursions.map((excursion, index) => (
                <tr key={excursion.id} className={index % 2 === 0 ? 'table-row-even' : 'table-row-odd'}>
...
export default OrganizerExcursionManagementTool;